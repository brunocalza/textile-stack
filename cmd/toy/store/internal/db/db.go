...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.listPeopleStmt, err = db.PrepareContext(ctx, listPeople); err != nil {
		return nil, fmt.Errorf("error preparing query ListPeople: %w", err)
	}
	return &q, nil
}
...
func (q *Queries) Close() error {
	...
	if q.listPeopleStmt != nil {
		if cerr := q.listPeopleStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing listPeopleStmt: %w", cerr)
		}
	}
	return err
}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		listPeopleStmt:   q.listPeopleStmt,
	}
}
